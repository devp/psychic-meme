 ...
 * @param {string} key
 * @param {string} value
 * @returns {boolean} false if the write didn't land (full, blocked)
 */
function writeRaw(key, value) {
  try {
    ...
    /* best effort -- but say so, so a caller holding words can warn */
    return false;
  }
}
...
export function recordStore(key) {
  ...
  function flush() {
    saved = writeRaw(RECORDS_KEY, JSON.stringify(records));
  ...
  const api = {
    /**
     * Did the last change reach storage? In-memory state is still correct
     * when it didn't; it just won't survive a reload.
     * @returns {boolean}
     */
    isSaved() {
      return saved;
